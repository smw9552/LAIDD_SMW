...
# PubChemPy 내부 urlopen을 SSL 검증 해제 버전으로 강제 교체
pcp.urlopen = lambda url, data=None, context=None: urllib.request.urlopen(
    url,
    data=data,
    context=ssl._create_unverified_context()
)
...
#분자 불러오기
c_list = pcp.Compound.from_cid(str(cid_single), context=context) #CID 값을 문자열로 입력해야 함
c_list = pcp.get_compounds('Eupatilin', 'name') #이름으로 검색
c_list = pcp.get_compounds('COC1=C(C=C(C=C1)C2=CC(=O)C3=C(O2)C=C(C(=C3O)OC)O)OC', 'smiles') #SMIELS로 검색
c_list = pcp.get_compounds('Eupatilin', 'name', record_type='3d') # 3D 구조 불러옴
